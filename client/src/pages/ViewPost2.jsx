...
function ViewPost2() {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`https://dummyjson.com/c/f94e-8502-4d8a-92cf`, {
      headers: {
  ...
  return (
    <div className='bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 py-3 px-10 lg:px-20 min-h-screen'>
      ...
      <div className="max-w-2xl mx-auto mt-8 bg-white rounded-2xl shadow-2xl p-8">
        ...
        <div className="mb-4">
          <h2 className="font-semibold text-lg mb-1">Video</h2>
          {post.video_url ? (
            <a
              href={post.video_url}
              target="_blank"
            ...
            <p className="text-gray-700 text-base">No video available.</p>
          )}
        </div>
        <div className="flex items-center gap-6 pt-3 border-t mb-4">
          <button
            className={`flex items-center gap-1 text-red-600 text-sm font-medium px-3 py-2 rounded-full ${showLikes ? 'bg-red-100' : ''}`}
            onClick={handleShowLikes}
          >
            <CiHeart className="text-3xl" /> Likes
          </button>
          <button
            className={`flex items-center gap-1 text-gray-500 text-sm font-medium px-3 py-2 rounded-full ${showComments ? 'bg-yellow-100' : ''}`}
            onClick={handleShowComments}
          >
            <IoChatboxOutline className="text-2xl" /> Comments
          </button>
        ...
        {showLikes && (
          <div className="mb-4">
            <h2 className="font-semibold text-lg mb-1">Likes ({post.likes?.length || 0})</h2>
            <div className="space-y-3 mb-4 max-h-40 overflow-y-auto">
              {post.likes?.length > 0 ? (
                post.likes.map((like, idx) => {
                  // If like is an object, display username or id
                  if (typeof like === 'object' && like !== null) {
                    return (
                      <div key={idx} className="text-sm">
                        <p className="font-medium">{like.user?.username || like.user || like.id || JSON.stringify(like)}</p>
                      </div>
                    );
                  }
                  // If like is a number or string, display as user id
                  return (
                    <div key={idx} className="text-sm">
                      <p className="font-medium">User ID: {like}</p>
                    </div>
                  );
                })
              ) : (
